"""Simulated annealing solver for edge scanwidth."""
...
class SimulatedAnnealingSolver(Solver):
    ...
    p_in : float, optional
        Initial acceptance probability. Default is 0.9.
    p_stop : float, optional
        Final acceptance probability. Default is 0.01.
    init_ext : Union[str, Extension], optional
        Initial extension. Either an :class:`Extension` or one of
        ``{"greedy", "cut", "random"}``.
    ...
    def solve(self, dag: DAG) -> SolverResult:
        ...
        -------
        SolverResult
            Standardized solver result including the trace of best
            scanwidths under ``history``.
        """
        graph = dag.graph
    ...
    def _initial_extension(self, dag: DAG) -> Extension:
        ...
        if self.init_ext == "cut":
            return CutSplittingSolver().solve(dag).extension
        ...
        raise ValueError(
            ...
            f"{{'greedy', 'cut', 'random'}}, got {self.init_ext}"
        )
...
def _random_neighbour_scanwidth(
    ...
    """Return scanwidth data for a random neighboring tree extension."""
    possible_choices = []
